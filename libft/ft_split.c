...
/*   Updated: 2023/06/21 18:26:57 by mde-sara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*void	ft_copy_letters(char *dest, const char *source, char c, int num)
{
	int	j;
	int	start_index;
	...
	while (j < num)
	{
		if (source[start_index = j] != c)
			dest[j] = source[start_index + j];
		j++;
	}
	dest[j] = '\0';
}*/
...
char	**ft_memword(char **memtot, char *str, char c, int num)
{
	...
	while (str[i])
	{
		...
		if (num > 0)
		{
			...
			//ft_copy_letters(memtot[word_index], str, c, num);
			word_len = 0;
			while (word_len < num)
			{
				if (str[i] != c)
					memtot[word_index][word_len] = str[i];
				i++;
				word_len++;
			}
			memtot[word_index][word_len] = '\0';
			word_index++;
		}
		else
			i++;
